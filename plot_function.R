...
#(1) Variable sorting
Attrition_data_predicted <- Attrition_data %>% select(- EmployeeCount, -EmployeeNumber, -Over18, -StandardHours)
...
#rcov$auc
#prop_detree <- prop.table(table(test$Attrition, preds, dnn = c("Actual", "Predicted")),1)
...
#rocrf$auc
#prop_randf <- prop.table(table(test$Attrition,rfpreds, dnn = c("Actual", "Predicted")),1)
...
jobinvolvement_prediction <- ggplot(data = test, aes(x = MonthlyIncome, y = Prediction)) +
  geom_point(aes(color = JobInvolvement1), alpha = 0.7) +
  geom_smooth(method = "lm") +
  facet_wrap(~ JobInvolvement1) +
  theme_economist() +
  theme(legend.position = "none") +
  labs(title = "JobInvolvement", x = "MonthlyIncome", y = "Possibility of leaving")

weightedprobs <- predict(weightedfit, test, type = "prob")
test$Prediction <- weightedprobs$Yes
department_prediction <-ggplot(data = test, aes(x = JobRole, y = Prediction)) +
